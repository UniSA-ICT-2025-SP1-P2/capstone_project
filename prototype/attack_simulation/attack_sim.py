...
def split_and_encode_data(df):
    ...
    Args:
        df (pd.DataFrame): Preprocessed DataFrame from load_and_preprocess_data.
    ...
    joblib.dump(le_catname, "le_catname.pkl")
...
def train_models(X_train, y_train, X_val, y_val, X_test, y_test, feature_cols, le_catname, train_df, meta_test):
    ...
    Args:
        ...
        le_catname (LabelEncoder): Encoder for category names.
    ...
    trained_models = {name: joblib.load(f"{name}_AA_model.pkl") for name in classifiers.keys()}
    return trained_models, shap_values_dict, metrics_df, shap_features_df

def load_trained_models(model_names=['LogisticRegression', 'DecisionTree', 'RandomForest']):
    """
    Load pre-trained models from disk.

    Args:
        model_names (list): List of model names to load (default: ['LogisticRegression', 'DecisionTree', 'RandomForest']).
    ...
    for name in model_names:
        try:
            model = joblib.load(f"{name}_AA_model.pkl")
            trained_models[name] = model
        except FileNotFoundError:
            print(f"Model file '{name}_AA_model.pkl' not found.")
...
def load_preprocessed_data():
    ...
    Returns:
        tuple: (X_train, y_train, X_val, y_val, X_test, y_test, feature_cols, le_catname, train_df, test_df)

    Raises:
        FileNotFoundError: If required files are missing.
    """
    required_files = [
        "Train_Dataset_Malware_Type.csv",
        "Validation_Dataset_Malware_Type.csv",
        "Test_Dataset_Malware_Type.csv",
        "feature_cols.pkl",
        "le_catname.pkl"
    ]
    for file in required_files:
        if not os.path.exists(file):
            raise FileNotFoundError(f"Required file '{file}' not found. Run split_and_encode_data first.")

    train_df = pd.read_csv("Train_Dataset_Malware_Type.csv")
    validation_df = pd.read_csv("Validation_Dataset_Malware_Type.csv")
    test_df = pd.read_csv("Test_Dataset_Malware_Type.csv")
    feature_cols = joblib.load("feature_cols.pkl")
    le_catname = joblib.load("le_catname.pkl")
    ...
    return X_train, y_train, X_val, y_val, X_test, y_test, feature_cols, le_catname, train_df, test_df
...
def fgsm_attack_simulation(X_test, test_df, le_catname, feature_cols, trained_models):
    ...
    Returns:
        dict: Adversarial samples for each epsilon value.
    """
    ...
    X_train_subset = X_test.sample(100, random_state=42) if len(X_test) > 100 else X_test
...
def generate_fgsm_adversarial_samples(X_train, y_train, le_catname, feature_cols, source_model='LogisticRegression', epsilon=0.1, n_samples=5000):
    ...
    Args:
        ...
        le_catname (LabelEncoder): Encoder for category names.
        feature_cols (list): List of feature column names.
        source_model (str): Model name to load and use for attack (default: 'LogisticRegression').
        epsilon (float): Perturbation strength (default: 0.1).
        n_samples (int): Number of adversarial samples to generate (default: 5000).

    Returns:
        pd.DataFrame: Adversarial samples with features and labels.
    """
    try:
...
def run_fgsm_attack_simulation(data_path=None):
    """
    Wrapper to run FGSM attack simulation, either with preprocessed data or from scratch.

    Args:
        data_path (str, optional): Path to raw data. If None, attempts to load preprocessed data.

    Returns:
        dict: Adversarial samples from fgsm_attack_simulation, or None if prerequisites fail.
    """
    if data_path is not None:
        ...
        trained_models, _, _, _ = train_models(
            X_train, y_train, X_val, y_val, X_test, y_test, feature_cols, le_catname, train_df, meta_test
        )
    else:
        try:
            X_train, y_train, X_val, y_val, X_test, y_test, feature_cols, le_catname, train_df, test_df = load_preprocessed_data()
            trained_models = load_trained_models(['LogisticRegression', 'DecisionTree', 'RandomForest'])
        except FileNotFoundError as e:
            print(f"Error: {e}. Please provide data_path to preprocess and train models.")
            return None
...
def main(data_path):
    """
    Execute the full malware analysis pipeline.

    Args:
        data_path (str): Path to the dataset CSV file.

    Returns:
        dict: Dictionary containing key outputs of the pipeline.
    """
    ...
    return {
        ...
        'fgsm_adv_samples': fgsm_adv_samples,
        'pgd_samples': pgd_samples,
        'pgd_adv_samples': pgd_adv_samples
    }

if __name__ == "__main__":
    data_path = "data.csv"  # Update with actual path
    results = main(data_path)
    print("Pipeline completed. Results available in the returned dictionary.")
